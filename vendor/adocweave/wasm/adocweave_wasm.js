/* @ts-self-types="./adocweave_wasm.d.ts" */

/**
 * @param {any} request
 * @returns {any}
 */
export function preprocess(request) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.preprocess(retptr, addHeapObject(request));
...
/**
 * @param {any} request
 * @param {Function | null} [cancellation]
 * @returns {any}
 */
export function process(request, cancellation) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.process(retptr, addHeapObject(request), isLikeNone(cancellation) ? 0 : addHeapObject(cancellation));
        var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
        var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
        var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
        if (r2) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('adocweave_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
